# -*- coding: utf-8 -*-
"""Colab-UnityEnvironment-2-Train.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/github/Unity-Technologies/ml-agents/blob/release_18_docs/colab/Colab_UnityEnvironment_2_Train.ipynb
...
class ActorDiscretePPO(nn.Module):
    def __init__(self, vec, enc, action_dim):
        super().__init__()
        mid_dim = 256
        state_dim = vec
...
class Trainer:
  ...
  def generate_trajectories(
    env: BaseEnv, q_net, buffer_size: int, epsilon: float
  ):
    """
    ...
    while len(buffer) < buffer_size:  # While not enough data in the buffer
      ...
      for agent_id_terminated in terminal_steps:
        # terminal_steps[agent_id_terminated].obss = [np.concatenate((terminal_steps[agent_id_terminated].obs[0], terminal_steps[agent_id_terminated].obs[1]))]
        ...
        last_experience = Experience(
          ...
          next_obs=np.concatenate((terminal_steps[agent_id_terminated].obs[0], terminal_steps[agent_id_terminated].obs[1])),
        )
      ...
      for agent_id_decisions in decision_steps:
        ...
        # decision_steps[agent_id_decisions].obss = [np.concatenate((decision_steps[agent_id_decisions].obs[0], decision_steps[agent_id_decisions].obs[1]))]
        ...
        if agent_id_decisions in dict_last_obs_from_agent:
          ...
          exp = Experience(
            ...
            next_obs=np.concatenate((decision_steps[agent_id_decisions].obs[0], decision_steps[agent_id_decisions].obs[1])),
          )
        ...
        dict_last_obs_from_agent[agent_id_decisions] = (
          np.concatenate((decision_steps[agent_id_decisions].obs[0], decision_steps[agent_id_decisions].obs[1]))
        )

      # Generate an action for all the Agents that requested a decision
      # Compute the values for each action given the observation
      actions_values = (
        q_net(torch.from_numpy(np.concatenate([decision_steps.obs[0], decision_steps.obs[1]],axis=1))).cpu().detach().numpy()
      )
  ...
  def update_q_net(
    ...
    for _ in range(NUM_EPOCH):
      for batch in batches:
        ...
        obs = torch.from_numpy(np.stack([ex.obs for ex in batch])).cuda()
        ...
        action = torch.from_numpy(np.stack([ex.action for ex in batch])).cuda()
        next_obs = torch.from_numpy(np.stack([ex.next_obs for ex in batch])).cuda()
        ...
        mask = torch.zeros((len(batch), action_size)).cuda()
...
NUM_TRAINING_STEPS = int(os.getenv('QLEARNING_NUM_TRAINING_STEPS', 270))
...
for n in range(NUM_TRAINING_STEPS):
  ...
  Trainer.update_q_net(qnet, optim, experiences, 7)
